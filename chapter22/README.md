...
[自定义事件对象](./22.4/EventTargetExample01.html)
[寄生组合继承自定义事件对象](./22.4/EventTargetExample02.html)

## 22.5 拖放（拖拽）
拖放是一种非常流行的用户界面模式。它的概念很简单：点击某个对象，并按住鼠标按钮不放，将鼠标移动到另一个区域，然后释放鼠标按钮将对象“放”在这里。

拖放的基本概念很简单：创建一个绝对定位的元素，使其可以用鼠标移动。设置一个 onmousemove 事件处理程序，它总是将指定元素移动到鼠标指针的位置，如下面的例子所示：[mousemove实现元素跟随鼠标](./22.5/DragAndDropExample01.html)

根据前面的示例，我们可以推断出拖拽的原理：当鼠标按钮按下的时候（mousedown），当鼠标移动的时候（mousemove），当鼠标松开释放的时候（mouseup）。通过这三个事件响应，控制元素的绝对定位和相对定位的切换。请看实现的例子：[实现元素拖拽功能](./22.5/DragAndDropExample02.html)

### 22.5.1 修缮拖动功能
当你试了上面的例子之后，你会发现元素的左上角总是和指针在一起，这是因为event.clientX指的是鼠标的坐标。我们可以再进一步优化，拖拽元素的时候，让鼠标指针保持不动。每当拖拽时，将元素和鼠标的坐标之间的差值计算出来，移动的时候将差值补上即可。

[实现元素拖拽功能-优化鼠标指针](./22.5/DragAndDropExample03.html)
